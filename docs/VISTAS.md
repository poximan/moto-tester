...
| Bombas | `PumpGrid.tsx`, `PumpCard.tsx` | `capabilities/pumps/pumps_api.py` | `pumps_service.py` | `RuntimeState` y `PumpControlStore`: Tablero/RTU y modo EMar compartidos |
...
`RuntimeState`, `SnapshotHub`, `PumpControlStore` y `EmulatorClient` son compartidos
porque representan una unica sesion Modbus, un unico stream, una unica seleccion
persistente de controles de bomba y un unico servicio experto. Separarlos por vista duplicaria la
fuente operativa y la correlacion de escrituras.
